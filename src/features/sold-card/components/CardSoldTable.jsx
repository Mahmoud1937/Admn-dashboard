          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {items.map((sold) => (
              <tr key={sold.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-sm text-center text-gray-700">{sold.clientName}</td>
                <td className="px-4 py-3 text-sm text-center text-gray-700">{sold.clientPhone}</td>
  <td className="px-4 py-3 text-sm text-center">
  <Link
    to={`/card-activation?cardPoolId=${sold.id}&sourceType=2`}
    className="text-blue-600 hover:text-blue-800 hover:underline"
  >
    {sold.active}
  </Link>
</td>
                <td className="px-4 py-3 text-sm text-center text-gray-700">{sold.nonActive}</td>
                <td className="px-4 py-3 text-sm text-center">
                  {sold.proofPayment ? (
                    <button
                      type="button"
